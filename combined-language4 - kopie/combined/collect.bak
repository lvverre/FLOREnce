...
(define (collect* logic-connector-names fact-name size collector )
  (displayln (format "WORL: ~a" logic-connector-names))
      (let* (
             (logic-connectors (map (lambda (var ) (lookup-local-var-error var global-env))
        ...
        (let ((collection-size (calculate-collection-size logic-connectors)))
          (displayln "collections size")
          (displayln collection-size)
