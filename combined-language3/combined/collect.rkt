...
(define (collect* logic-connector-names fact-name size collector root-env)
      (let* (
             (logic-connectors (map (lambda (var ) (lookup-var-error var global-env))
...
(define (add-collect* logic-connector-names fact-name time-interval size)
...
(define (remove-collect* logic-connector-names   fact-name size )
...
(define (interpret-collect stx )
  (syntax-parse stx
    [((~literal add-collect1:) logic-connector-names:id ... (~literal as:) fact-name:id (~literal for:) time-interval:number)
     (add-collect* (map syntax-e (syntax->list #'(logic-connector-names ...))) (syntax-e #'fact-name) (syntax-e #'time-interval)  2)]
    [((~literal add-collect2:) logic-connector-names:id ... (~literal as:) fact-name:id (~literal for:) time-interval:number)
     (add-collect* (map syntax-e (syntax->list #'(logic-connector-names ...))) (syntax-e #'fact-name) (syntax-e #'time-interval)  3)]
    [((~literal add-collect3:) logic-connector-names:id ... (~literal as:) fact-name:id (~literal for:) time-interval:number)
        (add-collect* (map syntax-e (syntax->list #'(logic-connector-names ...))) (syntax-e #'fact-name) (syntax-e #'time-interval)  4)]
    [((~literal add-collect4:) logic-connector-names:id ... (~literal as:) fact-name:id (~literal for:) time-interval:number)
     (add-collect* (map syntax-e (syntax->list #'(logic-connector-names ...))) (syntax-e #'fact-name) (syntax-e #'time-interval)  1)]
    [((~literal remove-collect4:) logic-connector-names:id ... (~literal as:) fact-name:id)
     (remove-collect* (map  syntax-e (syntax->list #'(logic-connector-names ...))) #'fact-name 4 )]
      [((~literal remove-collect3:) logic-connector-names:id ... (~literal as:) fact-name:id)
     (remove-collect* (map  syntax-e (syntax->list #'(logic-connector-names ...))) #'fact-name 3 )]
      [((~literal remove-collect2:) logic-connector-names:id ... (~literal as:) fact-name:id)
     (remove-collect* (map  syntax-e (syntax->list #'(logic-connector-names ...))) #'fact-name 2 )]
      [((~literal remove-collect1:) logic-connector-names:id ... (~literal as:) fact-name:id)
     (remove-collect* (map  syntax-e (syntax->list #'(logic-connector-names ...))) #'fact-name 1 )]))
