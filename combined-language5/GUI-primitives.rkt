         ...
         (let* ((event (event-node '()))
                (check-box (new class-check-box [co-x x][co-y y] [min-width min-width]
                               ...
                               [callback (lambda (check-box control-event)
                                           (fire  event (send check-box get-value)))][label label]))
...
(define n (new frame% [label "test"] [width 300] [height 399]))

(define z (new list-box% [label "Order:"] [choices (list )] [parent n][callback (lambda (s e)
                                                                                  (display (car (send s get-selections))))]))
(send n show #t)
