        ...
        (else
         (cond
           ...
           (else
            ...
            (set-dependency-node-idx! node temporary)
            (display (internal-node-predecessors node))
  ...
  (if (symbol? event-name)
      (let ((predecessor (lookup-local-var-error event-name env)))
        (cond (predecessor
               (display "MAP")
               (display predecessor)
               (make-single-function-node   predecessor (compile expr ) event-name))
              (else
...
(define (make-filter-node event-name expr env )
  (if (symbol? event-name)
      (let ((predecessor (lookup-local-var-error event-name env)))
        ...
        (cond (predecessor
               (filter-node -2 (list predecessor) (compile expr ) event-name))
              (else
               (error (format "~a is not defined" event-name)))))
...
(define (interpret-def-body stx env )
  (syntax-parse stx
    [((~literal map:) val:id body)
      (make-map-node (syntax-e #'val) #'body env )]
    [((~literal filter:) val:id body)
      (make-filter-node (syntax-e #'val) #'body env )]
    [((~literal or:) name-left:id name-right:id)
      (make-or-node (syntax-e #'name-left) (syntax-e #'name-right) env)]))

(define (interpret-reactor-body stx env input-vars)
  (syntax-parse stx
    [(((~literal def:) event-name:id node) ...)
     (display (append (map syntax-e (syntax->list #'(event-name ...)))
                                                           input-vars))
     (let ((duplicated-variable (check-duplicates (append (map syntax-e (syntax->list #'(event-name ...)))
                                                           input-vars))))

       (when duplicated-variable
         (error (format "~a is already defined" duplicated-variable))))
                        
         (for-each (lambda (event-name node)
                     (define-node (syntax-e event-name) (interpret-def-body  node env) env))
                   (syntax->list #'(event-name ...))
                   (syntax->list #'(node ...)))]))


(define (model-var-name? name)
  (let ((name-string (symbol->string name)))
    (equal? #\M (string-ref name-string 0))))
    ...
    [((~literal reactor:) name:id
     ...
     (with-syntax ([model-vars (if (attribute consts) #''(consts ...) #''())])
       #'(let* ((eval-name 'name)
           ...
           (when (check-duplicates (append model-vars eval-ins))
               ...
               (let* ((node-env (new-local-env))
                     ...
                     ; (model-env (make-hash (map (lambda (name name-with-id) (cons name (add-id-number name))) model-vars model-vars-with-id)))
                      (in-nodes (map (lambda (name)
                                     ...
                                     eval-ins)))
                 (interpret-reactor-body #'(exprs ... ) node-env (append model-vars eval-ins))
